...
> **Revision note (v16):** a code-review pass (backlog `TASK-002`, finding `REV-006`) found this document's §8.6 contract rules ("`CMP-004` only... never directly") directly contradicted the backlog's own `TASK-002` ticket, which explicitly authorises `CMP-014`/`CMP-015` (UI selector population) and `CMP-003` (geography resolver) to call `get_geography_reference`/`get_period_reference` directly — the implementation (`ui/_shared.py`) already matched the backlog's version, so this document's wording, not the code, was stale. The distinction the backlog draws is sound and is preserved here rather than tightened away: those two methods are pure, parameterless reference-data reads with no formula to duplicate or diverge (§6.7's "SQL selects, Python computes" split doesn't apply to a plain table read), unlike every other repository method, which does carry the risk `ADR-001`'s single-implementation principle exists to prevent. §5 (`CMP-002`'s Responsibility/Interfaces columns) and §8.6 (the Consumer line and the Contract rules bullet) are corrected in place to state the same exception the backlog already granted. Scope: documentation only — no schema, contract, or code change; `core/tools.py` (`CMP-004`) remains the sole caller for every method that does carry analytical logic (`get_price_series`, `get_premium_series`, `get_price_series_multi`).

> **Revision note (v17):** a code-review pass (backlog `TASK-005`, finding `REV-012`) found that `ADR-005`'s claim — DuckDB "expresses the new-build↔existing premium join **and multi-area ranking/comparison queries as single parameterised SQL statements**" — and this section's own "low single-digit milliseconds" performance claim (§12) did not hold for premium-based `rank_areas`/`compare_areas` calls: `get_premium_series` only ever took one area, so `core/tools.py` issued one call per area in a Python loop for premium metrics, unlike the batched `get_price_series_multi` path. Measured (real bundled snapshot, warm connection, 5 repeated runs): a full-scope (318-area) premium ranking averaged ~0.40s versus ~0.003s for the equivalent price-based ranking — a genuine, reproducible ~150x gap, not the code's comments alone. This revision adds `get_premium_series_multi` (§8.6): the same self-join as `get_premium_series`, widened from `nb.la_code = ?` to `nb.la_code = ANY(?)`, mirroring `get_price_series_multi`'s existing pattern exactly — no new schema, no new join shape, no application-side merge (preserving §6.7's existing rule). `core/tools.py`'s `_premium_level_values`/`_premium_range_values` now call it once per `rank_areas`/`compare_areas` invocation instead of once per area; `get_premium_series` itself, and every function that calls it (`new_build_premium`, `premium_trend`, `premium_series` — all single-area, `TASK-004`), are unchanged. Scope: `CMP-002`/`core/repository.py`, §8.6 only; no schema, contract, or behaviour change to any TASK-004 function, `RankingResult`/`ComparisonResult`, or their callers.
...
Unresolved decisions carried forward (none blocking, see §16): exact tested default model ID, to be confirmed by `SPIKE-001` and documented per `ADR-007` (v4). The mixed-coverage partial-answer policy (`ADR-010`) was confirmed by the stakeholder on 2026-08-13, matching the designer's original recommendation and requiring no implementation change.
...
| CMP-002 | DuckDB Repository **(v3, renamed from "Dataset Store"; `ADR-005` superseded)** | Opens an embedded DuckDB connection once at app startup and registers read-only views directly over the processed Parquet files (`CREATE VIEW ... AS SELECT * FROM read_parquet(...)`, no copy into DuckDB's own storage); exposes a small, fixed set of parameterised repository methods (§8.6), the analytical ones called by `CMP-004` and **(v16, corrected — matches the backlog's `TASK-002` scope, previously understated here)** two pure reference-data methods (`get_geography_reference`, `get_period_reference`) also called directly by `CMP-014`/`CMP-015`/`CMP-003` — never raw SQL text, never a live connection object, passed to any caller | Parquet files (views); parameterised query arguments from `CMP-004` and, for the two reference-data methods, `CMP-014`/`CMP-015`/`CMP-003` | Typed records/Pydantic objects only (e.g. `list[PricePoint]`) — **(v3) fixed from the v1/v2 design, which stated raw `DataFrame`s as this component's output; that was exactly the kind of Pandas-type leakage `CON-009` flags, closed here** | Owns the DuckDB connection and its views (read-only; no runtime write path) | Python function calls from `core.tools` (`CMP-004`) for every analytical method, plus direct calls to `get_geography_reference`/`get_period_reference` from `CMP-014`/`CMP-015`/`CMP-003` **(v16)** — no other component holds a DuckDB handle | CMP-001 output, `duckdb` | Refuses to start if snapshot missing/checksum mismatch — explicit startup error, not a silent empty dataset | DR-001–DR-007, NFR-001, CON-008, CON-009 |
...
**Owner:** `CMP-002` (`core/repository.py`). **Consumer:** `CMP-004` for every analytical method. **(v16, corrected)** `get_geography_reference`/`get_period_reference` are the one stated exception — pure, parameterless reference-data reads with no formula to duplicate, called directly by `CMP-014`/`CMP-015` (UI selector population) and `CMP-003` (geography resolver alias table), per the backlog's `TASK-002` Implementation notes. No other component imports `core.repository` or holds the DuckDB connection.
...
| `get_premium_series_multi` **(v17, new)** | `(la_codes: list[str], period_start: date, period_end: date) -> list[PremiumRow]` | Same self-join as `get_premium_series`, widened to `nb.la_code = ANY(?)` — fixed query text regardless of how many codes are passed | Powers `rank_areas`/`compare_areas`'s premium-based metrics in one round trip; closes `REV-012` (`TASK-005` review), which measured the prior per-area-loop pattern at ~150x slower for a full-scope ranking |
...
- `CMP-004` is the caller for every method that carries analytical logic (`get_price_series`, `get_premium_series`, `get_price_series_multi`, **(v17)** `get_premium_series_multi`); the agent's tool wrappers (`CMP-005`) reach these exclusively through `CMP-004`, never directly — preserving the same call-graph shape as the pre-v3 design (§4), just with `CMP-002`'s internals changed. **(v16, corrected)** `get_geography_reference`/`get_period_reference` are the one stated exception: they carry no analytical logic to duplicate or diverge, so `CMP-014`/`CMP-015` (UI selector population) and `CMP-003` (geography resolver) call them directly — this was already the implemented and tested behaviour (backlog `TASK-002`); this bullet's wording was simply never reconciled with it until now.
...
| ADR-010 **(confirmed, 2026-08-13)** | Mixed-coverage requests (e.g. Glasgow + Edinburgh + Manchester) get a partial, caveated answer — full analysis for covered areas, explicit non-fabrication statement for uncovered ones — rather than a blanket refusal or silent substitution | Accepted, stakeholder-confirmed | FR-009 usefulness vs. FR-012 non-fabrication, directly triggered by example Q5/Q6 | Blanket refusal of the whole request; silent substitution of a nearby covered area; partial answer with explicit caveat (**selected, stakeholder-confirmed**) | Maximises usefulness (a real assessor question referencing Glasgow shouldn't return nothing at all) while preserving grounding (no invented Scottish figures, no silent substitution) | None outstanding — the stakeholder confirmed the designer's recommendation as-is during Increment 4, matching what STORY-007 already implements; no code change required | FR-009, FR-012, ADR-006 |
...
| Resolved (2026-08-13) | ADR-010's partial-answer-vs-full-refusal policy for mixed-coverage requests — confirmed by the stakeholder as partial-answer-with-caveat, matching the designer's original recommendation and the existing STORY-007 implementation | None — closed | Partial-answer-with-caveat confirmed as final; no code or behaviour change required | CMP-009 | FR-009, FR-012 | Resolved during increment 4 |
...
1. ~~Run `SPIKE-001` against the assessment's provisioned key to confirm a tested default model...~~ **Resolved 2026-08-13**: `SPIKE-001` ran against the real provisioned key and confirmed `gpt-4o-mini` (access, function calling, structured outputs, challenge-restriction compliance all verified); documented as `TESTED_DEFAULT_MODEL` in `agent/config.py` and in the README.
2. ~~Confirm or override ADR-010's partial-answer-with-caveat policy for mixed-coverage requests...~~ **Resolved 2026-08-13**: stakeholder confirmed partial-answer-with-caveat, matching the designer's original recommendation and the existing implementation (`STORY-007`) unchanged.
